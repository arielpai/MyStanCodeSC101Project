...
def get_best_pixel(pixels):
    ...
    minimum = float('inf')
    for pixel in pixels:
        # dist = ((pixel.red-rgb[0])**2 + (pixel.green-rgb[1])**2 + (pixel.blue-rgb[2])**2)**(1/2)
...
def solve(images):
    ...
    for b_pixel in result:
        p_list = []
        for image in images:
            img_pixel = image.get_pixel(b_pixel.x, b_pixel.y)
            p_list.append(img_pixel)
        best = get_best_pixel(p_list)
        b_pixel.red = best.red
        b_pixel.green = best.green
        b_pixel.blue = best.blue
